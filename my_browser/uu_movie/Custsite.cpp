												  ...
												  LRESULT * plResult)
{
	...
	//kgs
	if (theApp.m_Rose_Count >= 0)
	{
		CString s = (CString)lpstrText;
		//
		if (s.Find("지급") != -1) //찾을때만 
			theApp.m_Rose_Count++;
		if (s.Find("받을") == -1) //못찾을때만,그외 메시지 
			theApp.m_Rose_Text = s;
			;//MessageBox(hwnd,(CString)lpstrText,  "true", /*dwType*/MB_ICONERROR);
	}
